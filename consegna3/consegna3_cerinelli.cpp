...
int maxCycle = 0; //variabile per tenere il ciclo massimo (fino a quel momento)
int indexCycle = 0; //ciclo massimo di quel determinato nodo
...
int has_cycle(int n, int inizio) {
    ...
    if (V_visitato[n] == 1){
        ...
        if( n == inizio && indexCycle > maxCycle){ //SE il nodo è uguale a quello iniziale ho trovato un ciclo!
                                                   //e se il ciclo corrente è > del max aggiorno
            maxCycle = indexCycle;
            stackMaxCycle = stackCycle; //mi salvo i nodi che lo compongono
        }
    ...
    if (V_visitato[n] == 2) // trovato un nuovo percorso alternativo (non c'e' il ciclo)
        return 0;  

//SONO DENTRO UN NODO NUOVO

    indexCycle++; //aumento contatore ciclo locale
    stackCycle.push(n); //metto dentro uno stack il nodo nuovo
    ...
    int t = 0;
    node_t *elem = E[n]->head;
    while (elem != NULL) { /// espolo tutti i nodi nella lista di adiacenza di n
        ...
        //Quando torno al nodo inziale risetto gli altri nodi come "mai visitati"
        //questo perchè ci sono casi in cui un nodo veniva segnato come "TERMINATO" anche se non avevo
        //ancora esplorato tutti i possibili percorsi. (es: grafo nel main partendo dal nodo 0:
        //vado in 4, poi in 3, poi in 2 (che viene terminato), torno in 3 e finisco il giro.
        //quando poi ritorno nel 4 alla fine della ricorsione e esploro il 2, lo trovavo già terminato e lo saltavo.
        //questo mi faceva perdere potenziali percorsi)
        if(n==inizio) 
        {             
            for(int i =0; i<=n_nodi;i++){
                if (i == n) continue;
    ...
    if(!stackCycle.empty()){
            stackCycle.pop(); //pop del nodo corrente dallo stack temporaneo
    }
    indexCycle--; // mando indietro di uno l'index del ciclo
...
int main(int argc, char **argv) {
    ...
    std::stack<int> tempStack; //STAMPA per elencare i nodi in ordine giusto e non al contrario
